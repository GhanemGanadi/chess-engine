...
class MoveGeneration {
    ...
    static U64 Generate_Knight_Moves(const int square) {
    ...
    static U64 Generate_King_Moves(const int square) {
    ...
    static U64 Generate_Pawn_Moves(const int square, const PieceColour colour) {
        const int direction = colour == WHITE ? -8 : 8;
        if((square + direction) > 63 || (square + direction) < 0) return 0ULL;
        U64 pawn = 1ULL << (square + direction);

        bool onStartRank = colour == WHITE ? (square >= 48 && square  <= 55) : (square >= 8 && square <= 15);
    ...
    static U64 Generate_Pawn_Attacks(const int square, const PieceColour colour) {
        U64 attacks = 0ULL;
    ...
    static U64 Generate_Pawn_En_Passant(const int square, const PieceColour colour, const Board& board) {
        U64 moves = 0ULL;
        ...
        if(colour == WHITE && fromRank != 3) return moves;  // 5th rank
        if(colour == BLACK && fromRank != 4) return moves;  // 4th rank
        ...
        // For white pawns moving up (-8) and black pawns moving down (+8)
    ...
    [[nodiscard]] U64 Get_Pseudo_Legal_Moves(const int square, const PieceColour colour, const PieceType pieceType,
    ...
    static U64 Filter_Legal_Moves(U64 moves, const int square, const PieceColour colour,
...
public:
    ...
    [[nodiscard]] U64 Get_King_Moves(const int square) const {return kingMoves[square];}
    [[nodiscard]] U64 Get_King_Castle_Moves(const PieceColour colour, const Board& board) const {
        U64 moves = 0ULL;
    ...
    [[nodiscard]] U64 Get_Knight_Moves(const int square) const {return knightMoves[square];}
    [[nodiscard]] U64 Get_Pawn_Moves(const int square, const PieceColour colour) const {return pawnMoves[colour == WHITE ? 0 : 1][square];}
    [[nodiscard]] U64 Get_Pawn_Attacks(const int square, const PieceColour colour) const {return pawnAttacks[colour == WHITE ? 0 : 1][square];
    }


    U64 Get_Legal_Moves(const int square, const PieceColour colour, const PieceType pieceType, Board &board) const {
    ...
    std::vector<Move> Generate_All_Moves(const PieceColour colour, Board& board) const {
    ...
    private:
        void Initialise_Lookup_Tables() {
            ...
            for (int square = 0; square < 64; square++) {
                ...
                pawnMoves[0][square] = Generate_Pawn_Moves(square, WHITE);   // White
                pawnMoves[1][square] = Generate_Pawn_Moves(square, BLACK);  // Black
                pawnAttacks[0][square] = Generate_Pawn_Attacks(square, WHITE);   // White
                pawnAttacks[1][square] = Generate_Pawn_Attacks(square, BLACK);  // Black
            }
        }
};
